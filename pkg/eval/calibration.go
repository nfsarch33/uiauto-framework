...
// MeanBriers computes the two Brier means over graded decisions, kept
...
func MeanBriers(records []RunRecord) (choice, noul float64) {
...
func multiclassBrier(d DecisionRecord) float64 {
	dist := d.Dist
	if dist == nil {
		// No distribution reported: treat as one-hot on the model's pick.
